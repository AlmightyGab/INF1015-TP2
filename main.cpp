...
Jeu* lireJeu(istream& fichier)
{
 ...
 ListeJeux creerListeJeux(const string& nomFichier)
 {
 	...
 	for ([[maybe_unused]] int n : iter::range(nElements)) {
		...
		for (unsigned i = 0; i < jeu->designers.nElements; i++) {
	 		Designer* designer = jeu->designers.elements[i];
	 		Designer* designerTrouve = trouverDesigner(span<Jeu*>(listeJeux.elements, listeJeux.nElements), designer->nom); //existe 
...
#pragma endregion
	...
	Designer* designer = trouverDesigner(span<Jeu*>(listeJeux.elements, listeJeux.nElements), "Rob Pardo"); //existe
	afficherInfoDesigner(designer);
	cout << ligneSeparation << endl;
	trouverDesigner(span<Jeu*>(listeJeux.elements, listeJeux.nElements), "Ab Cde"); //existe pas
	...
	// Exemple avec le premier dev
	cout << "A titre d'exemple, nous n'allons qu'utiliser le premier developpeur de la liste pour tester les fonctions." << endl;
	Developpeur* dev1 = listeDev.obtenirElements()[0];
	// Ajout de jeux au dev1
	cout << "Le premier developpeur de la liste est : " << dev1->obtenirNom() << endl;
	dev1->miseAJourJeux(listeJeux);
	dev1->afficherJeux();
	cout << "Nombre de jeux : " << dev1->obtenirNombreJeux();
	...
	for (unsigned i = 0; i < listeDev.obtenirNombreElements(); i++) {
		listeDev.retirerDeveloppeur(listeDev.obtenirElements()[i]);
	}
